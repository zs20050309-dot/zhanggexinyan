...
export default function ChatPage() {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#080808] flex flex-col items-center justify-center gap-4">
        ...
        <p className="text-sm text-neutral-600">AI 正在为你生成个性化问题...</p>
      </div>
    )
  }
  ...
  return (
    <div className="min-h-screen bg-[#080808] flex flex-col">
      ...
      {/* Navbar */}
      <nav className="shrink-0 h-16 border-b border-white/[0.06] bg-[#080808]/90 backdrop-blur-xl">
        ...
        <div className="w-full max-w-lg">
          ...
          <AnimatePresence mode="wait">
            ...
            >
              {/* Question */}
              <div className="mb-8">
                <p className="text-xs font-semibold text-amber-400/60 tracking-widest uppercase mb-4">
                  问题 {current + 1}
                </p>
                  ...
                  {currentQ.options.map((opt) => {
                    ...
                    return (
                      <button
                        ...
                        className={`group relative w-full text-left px-5 py-4 rounded-2xl border text-sm font-medium transition-all duration-200 active:scale-[0.98]
                          ${sel
                            ? 'bg-amber-500/12 border-amber-500/50 text-white shadow-[0_0_20px_rgba(245,158,11,0.08)]'
                      ...
                      >
                        <div className="flex items-center gap-3">
                          <span className={`w-4 h-4 rounded-full border-2 flex items-center justify-center shrink-0 transition-colors ${
                            ...
                            {sel && <span className="w-1.5 h-1.5 rounded-full bg-black" />}
                          </span>
                          {opt}
                        </div>
                      </button>
                ...
                <div>
                  <textarea
                    ...
                    className="w-full px-5 py-4 rounded-2xl bg-[#0f0f0f] border border-white/[0.07]
                               ...
                               focus:border-amber-500/40 focus:bg-[#111] transition-all duration-200"
                  />
                  <p className="text-xs text-neutral-700 mt-2 text-right">
                    {navigator.platform.includes('Mac') ? '⌘' : 'Ctrl'} + Enter 继续
                  </p>
                </div>
              )}
            </motion.div>
            ...
            >
              ...
              <div className="flex flex-col gap-2">
                {questions.slice(0, current).map((q, qi) => (
                  ...
                  >
                    <span className="text-[11px] text-neutral-700 shrink-0 mt-0.5 tabular-nums">Q{qi + 1}</span>
